...
#https://pypi.org/project/tinysegmenter/ - найденный токенизатор(вроде хороший), его офф сайт - http://tinysegmenter.tuxfamily.org/
...
def tokenize_stopping(sentence):
    ...
    stop_re = r'[\u3041-\u3096]'
    tokens_list = []
    tokenized_string = ' | '.join(segmenter.tokenize(sentence))
    ...
    tokenized_stopped_string = ' | '.join(tokens_list)
    return [tokenized_string, tokenized_stopped_string]
...
#здесь токенизация содержится не отдельной функцией(надо зафиксить)
